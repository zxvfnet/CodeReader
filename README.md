...
| プロジェクト | 内容 |
|---|---|
| `PdfBookmarkEditor.Core` | しおり操作ロジック(iText使用、クロスプラットフォーム) |
| `PdfBookmarkEditor.App` | WPF GUI(Windows専用) |
| `PdfBookmarkEditor.Core.Tests` | xUnit 単体テスト |

### 主な機能

- PDFを開いて既存しおりを階層ツリー表示(名前付き宛先もページ番号に解決)
...
テストはiTextでテスト用PDF(しおり付き・名前付き宛先・暗号化など)を生成し、
...
PDF処理に [iText](https://itextpdf.com/)(AGPLライセンス)を使用しています。
本アプリを改変してクローズドソースで商用配布する場合は、iTextの商用ライセンスが必要です。
